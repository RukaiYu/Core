...
static int _run(int argc, _TCHAR* argv[], _TCHAR* vmArgs[])
{
    ...
    if (g_pTangram->launchMode == LAUNCH_JNI) {
		...
		//CString strFile = CString(szPath);
		//int nPos = strFile.ReverseFind('\\');
		//CString str = strFile.Left(nPos + 1);
		//str += _T("tangram.jar");
		//strFile = CString(jarFile);
		//strFile += _T(";");
		//strFile += str;

		/* JNI launching, classpath is set using -Djava.class.path */
		//cp = strFile.GetBuffer();
		//cp = (_TCHAR *)malloc((_tcslen(CLASSPATH_PREFIX) + strFile.GetLength() + 1) * sizeof(_TCHAR));
    ...
    while(running)
    {
		...
		{
			//CString strFile = jarFile;
			//int nPos = strFile.Find(_T("\\.."));
			//if (nPos != -1)
			//{
			//	//C:\chromium\src\out\Default\obj\chrome\.. / .. / .. / .. / out / Default / plugins\org.eclipse.equinox.launcher_1.4.0.v20161219 - 1356.jar
			//	strFile = strFile.Mid(nPos + 1);
			//}
			//javaResults = startJavaVM(jniLib, vmCommandArgs, progCommandArgs, L"C:\\chromium\\src\\out\\Default\\plugins\\org.eclipse.equinox.launcher_1.4.0.v20161219-1356.jar");
			//javaResults = startJavaVM(jniLib, vmCommandArgs, progCommandArgs, strFile.GetBuffer());
