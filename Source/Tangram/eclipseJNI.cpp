...
JavaResults * startJavaJNI( _TCHAR* libPath, _TCHAR* vmArgs[], _TCHAR* progArgs[], _TCHAR* jarFile )
{
	...
	for(i = 0; i < numVMArgs; i++){
		...
		//		//GetModuleFileName(GetModuleHandle(L"TangramCore.dll"), szPath, MAX_PATH);
		...
		//}
		///end Add by Tangram Team/////
		//options[i].optionString = "C:\\chromium\\src\\out\\Default\\plugins\\org.eclipse.equinox.launcher_1.4.0.v20161219-1356.jar";
	...
	if( createJavaVM(&jvm, &env, &init_args) == 0 ) 
	{
		...
		if (g_pTangram)
		{
			if (m_pTangramApplicationImpl)
			{
				m_pTangramApplicationImpl->m_pJVM = jvm;
				m_pTangramApplicationImpl->m_pJVMenv = env;
				if (env)
				{
					m_pTangramApplicationImpl->systemClass = env->FindClass("java/lang/System");
					try
					{
						if (m_pTangramApplicationImpl->systemClass != nullptr)
						{
							m_pTangramApplicationImpl->exitMethod = env->GetStaticMethodID(m_pTangramApplicationImpl->systemClass, "exit", "(I)V");
							m_pTangramApplicationImpl->loadMethod = env->GetStaticMethodID(m_pTangramApplicationImpl->systemClass, "load", "(Ljava/lang/String;)V");
						}
					}
					catch (...)
					{
		...
		registerNatives(env);
		USES_CONVERSION;
		mainClassName = W2A(g_pTangram->InitEclipse(jarFile));
		...
		//OutputDebugString(_T("Error createJavaVM\n"));
		OutputDebugString(results->errorMessage);
		OutputDebugString(_T("\n"));
	}
...
void cleanupVM(int exitCode) {
	...
	if (shouldShutdown(env)) {
		if (g_pTangram&&m_pTangramApplicationImpl)
		{
			...
			for (auto it : g_pTangram->m_mapBKFrame)
			{
				...
				g_pTangram->GetWndFrame((__int64)::GetParent(hWnd),&pFrame);
				CWndFrame* _pFrame = (CWndFrame*)pFrame;
				if(_pFrame)
					_pFrame->m_pBKWnd = nullptr;
				::DestroyWindow(::GetParent(it.first));
				//::PostMessage(::GetParent(it.first),WM_DESTROY,0,0);
			}
		...
		else
		{
			...
			{
				if (systemClass != NULL) {
					//exitMethod = env->GetStaticMethodID(systemClass, "loadlibrary", "(Ljava/lang/String)V");
