...
{
	if (m_pTangramTabCtrl->m_pFrame == nullptr)
	{
		...
		if (pPage)
		{
			auto it = pPage->m_mapFrame.find(m_pTangramTabCtrl->m_ListCtrl.m_hWnd);
			if (it == pPage->m_mapFrame.end())
			{
				pPage->CreateFrame(CComVariant(0), CComVariant((__int64)m_pTangramTabCtrl->m_ListCtrl.m_hWnd), CComBSTR("HostListView"), &pFrame);
				if (pFrame)
					m_pTangramTabCtrl->m_pFrame = (CWndFrame*)pFrame;
			}
		}
	}
	...
	CString strFile = m_strDir + m_strSubDir + _T("\\") + GetItemText(pNMLV->iItem, 0) + _T(".html");
	CString strPath = g_pTangram->m_strAppCommonDocPath;
	IWndNode* pNode = nullptr;
	if (::PathFileExists(strFile))
	{
		strPath += _T("templatedetail.xml");
		m_pTangramTabCtrl->m_pFrame->Extend(CComBSTR("HostListView"), CComBSTR(strPath), &pNode);
...
void CTangramListCtrl::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (g_pTangram->m_pMDIMainWnd == nullptr&&g_pTangram->m_pActiveMDIChildWnd)
	{
		if (::IsChild(g_pTangram->m_pActiveMDIChildWnd->m_hWnd, g_pTangram->m_hTemplateWnd))
		{
			...
			::ShowWindow(g_pTangram->m_pActiveMDIChildWnd->m_pFrame->m_pWorkNode->m_pHostWnd->m_hWnd, SW_SHOW);
					...
					else
					{
						...
						if (it2 == g_pTangram->m_mapTangramAppProxy.end())
						{
							if (::LoadLibrary(it->second->m_strLib))
							{
								...
								if (it2 != g_pTangram->m_mapTangramAppProxy.end())
								{
									pProxy = (CTangramAppProxy*)it2->second;
								}
							}
						}
						else
						...
						{
							IWndNode* pNode = nullptr;
							CWndFrame* pFrame = m_pTangramTabCtrl->m_pWndNode->m_pTangramNodeCommonData->m_pFrame;
							pFrame->Extend(pFrame->m_strLastKey.AllocSysString(), CComBSTR(_T("")), &pNode);
	...
	if (nItem == -1)
	{
		::GetClassName(pFrame->m_hWnd, g_pTangram->m_szBuffer, MAX_PATH);
		CString strName = g_pTangram->m_szBuffer;
		pFrame->m_bChromeFrame = strName == _T("Chrome Extended Window Class");
		CString strKey = pFrame->m_strLastKey;
		if (pFrame->m_pWebWnd)
		{
			if (strKey == _T(""))
			{
			...
			pFrame->Extend(strKey.AllocSysString(), CComBSTR(((ChromePlus::CChromeTangram*)g_pTangram)->m_strDefaultXml), &pNode);
		}
		else
			pFrame->Extend(strKey.AllocSysString(), CComBSTR(_T("")), &pNode);
		return;
	}
	...
	case 0:
	{
		HWND hWnd = pFrame->GetParent().m_hWnd;
...
void CTangramTabCtrl::FillListCtrl()
{
	...
	if (::PathFileExists(strPath))
	{
		...
		{
			m_pFrame->Extend(CComBSTR("HostListView"), CComBSTR(strPath), &pNode);
		...
		else
		{
			...
			if(m_pFrame)
				m_pFrame->Extend(CComBSTR("default"), CComBSTR(strPath), &pNode);
		}
	}
}
