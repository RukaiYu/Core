...
namespace OfficePlus
{
	namespace OutLookPlus
	{
		...
		STDMETHODIMP COutLookAddin::SendUCMAMessage(BSTR bsipFrom, BSTR bsipTo, IDispatch* SenderDispObj, IWndNode* pSender, BSTR bstrMsg)
		{
			if (m_mapUCMABot.size())
			{
				...
				if (it != m_mapUCMABot.end())
				{
					...
					if (strTo.Find(_T("sip:")) != 0)
						strTo = _T("sip:") + strTo;
					LONGLONG h = 0;
					BOOL bOutLookObj = false;
					CComQIPtr<_Explorer> _pExplorer(SenderDispObj);
					if (_pExplorer)
					{
						bOutLookObj = true;
					}
					...
					{
						CComQIPtr<_Inspector> _pInspector(SenderDispObj);
						if(_pInspector)
							bOutLookObj = true;
					}
					if (bOutLookObj)
					{
						CTangramOutLookMessageObj* pObj = new CComObject<CTangramOutLookMessageObj>;
		...
		void COutLookExplorer::OnFolderSwitch()
		{
			...
			if (_pFolder)
			{
				...
				if (strXml != _T(""))
				{
					...
					g_pTangram->GetWndFrame((LONGLONG)_hwnd, &pFrame);
					if (pFrame == nullptr)
					{
						IWndPage* pPage = nullptr;
						g_pTangram->CreateWndPage((LONGLONG)m_hOutLookToday, &pPage);
		...
		void COutLookExplorer::SetDesignState()
		{
			...
			IWndFrame* pFrame = nullptr;
			g_pTangram->GetWndFrame((LONGLONG)pWnd->m_hWnd, &pWnd->m_pFrame);
			if (pWnd->m_pFrame == nullptr)
			{
				g_pTangram->CreateWndPage((LONGLONG)::GetParent(pWnd->m_hWnd), &pWnd->m_pPage);
				if (pWnd->m_pPage)
						...
						{
							{
								...
								if (pCtrlDisp)
								{
									...
									{
										HRESULT hr = g_pTangram->CreateWndPage((LONGLONG)hPWnd, &m_pTaskPanePage);
										if (m_pTaskPanePage)
										{
											m_pTaskPanePage->CreateFrame(CComVariant(0), CComVariant((LONGLONG)hWnd), CComBSTR(L"TaskPane"), &m_pTaskPaneFrame);
											if (m_pTaskPaneFrame)
											{
												IWndNode* pNode = nullptr;
		...
		void COutLookInspector::ActivePage()
		{
			...
			if (it2 != m_mapOutLookPage.end())
			{
				...
				if (m_pActiveOutLookPage->m_bDesignState)
				{
					...
					if (pAddin->m_pDesigningFrame != pFrame)
					{
						pAddin->m_pDesigningFrame = pFrame;
			...
			if (it == pAddin->m_mapOutLookPlusExplorerMap.end())
			{
				...
				if (m_pOutLookInspector->m_hWnd)
				{
					...
					if (m_hFrameWnd != hFrame)
					{
						...
						if (m_pFrame)
						{
							m_pFrame->ModifyHost((LONGLONG)hFrame);
						}
					}
				}
				if (m_strXml!=_T(""))
				{
					...
					if (m_pPage == nullptr)
					{
						g_pTangram->CreateWndPage((LONGLONG)m_hWnd, &m_pPage);
						if (m_pPage)
						{
					...
					if (m_pFrame)
					{
						...
						((CWndFrame*)m_pFrame)->HostPosChanged();
					}
		...
		void COutLookPageWnd::DesignPage()
		{
			if (m_bDesignState == false)
			{
				...
				if (m_pPage == nullptr)
				{
					...
					{
						BSTR bstrName = m_strName.AllocSysString();
						m_pPage->CreateFrame(CComVariant(0), CComVariant((LONGLONG)m_hFrameWnd), bstrName, &m_pFrame);
						if (m_pFrame)
						{
							...
							m_pFrame->Extend(bstrName, m_strXml.AllocSysString(), &pNode);
							g_pTangram->m_pDesigningFrame = (CWndFrame*)m_pFrame;
		...
		LRESULT CInspectorContainerWnd::OnTangramMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
		{
			if (m_pFrame&&wParam == 4096)
			{
				...
				m_pFrame->ModifyHost((LONGLONG)m_hFrameWnd);
			}
			return  DefWindowProc(uMsg, wParam, lParam);
		}
		...
		LRESULT CInspectorContainerWnd::OnItemLoad(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL&)
		{
			if (m_pFrame)
			{
				...
				m_pFrame->ModifyHost((LONGLONG)::CreateWindowEx(NULL, _T("Tangram Window Class"), _T(""), WS_CHILD, 0, 0, 0, 0, (HWND)hwnd, NULL, AfxGetInstanceHandle(), NULL));
				::DestroyWindow(hwnd);
				m_pFrame = nullptr;
				m_pPage = nullptr;
			}
			if (m_strXml != _T(""))
			{
				...
				if (m_Parse.LoadXml(m_strXml))
				{
					m_hFrameWnd = ::GetWindow(m_hWnd, GW_CHILD);
					g_pTangram->CreateWndPage((LONGLONG)m_hWnd, &m_pPage);
					m_pPage->CreateFrame(CComVariant(0), CComVariant((LONGLONG)m_hFrameWnd), CComBSTR(L"Default"), &m_pFrame);
					if (m_pFrame)
					{
						IWndNode* pNode = nullptr;
		...
		CTangramOutLookMessageObj::CTangramOutLookMessageObj()
		{
		...
		void CTangramOutLookMessageObj::ProcessMsg(CString strMsg)
		{
