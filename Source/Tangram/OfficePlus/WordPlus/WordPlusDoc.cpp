	...
	namespace WordPlus
	{
		...
		void CWordDocument::InitDocument()
		{
			...
			auto t = create_task([pAddin,pStream, pStream2, this]()
			{
				...
				if (m_strDocXml == _T(""))
				{
					m_strDocXml = pAddin->GetDocXmlByKey(_pDoc, CComBSTR(L"Tangrams"));
				}
				else
				{
					pAddin->AddDocXml(_pDoc, CComBSTR(m_strDocXml), CComBSTR(L"Tangrams"));
				}
				CoUninitialize();
			}).then([pAddin,this]()
			{
		...
		void CWordObject::OnObjDestory()
		{
			if (m_pOfficeObj != nullptr)
			{
				...
				if (nCount > 0 && m_pWordPlusDoc->m_pFrame)
				{
					...
					m_pWordPlusDoc->m_pFrame->ModifyHost((LONGLONG)::GetParent(it2->second->m_hChildClient));
					if (m_hTaskPane)
					{
						m_hTaskPane = NULL;
						m_pWordPlusDoc->m_pTaskPaneFrame->ModifyHost((LONGLONG)it2->second->m_hTaskPaneChildWnd);
				...
				if (nCount == 0)
				{
					...
					{
						::PostMessage(g_pTangram->m_hHostWnd, WM_TANGRAMMSG, (WPARAM)m_pWordPlusDoc->m_pFrame->m_pWorkNode->m_pHostWnd->m_hWnd, 1992);
					}
					HRESULT hr = S_OK;
