	...
	namespace PowerPointPlus
	{
		...
		void CPowerPntObject::OnObjDestory()
		{
			...
			if (m_pPresentation)
			{
				...
				if (nCount > 0 && m_pPresentation->m_pFrame)
				{
					...
					m_pPresentation->m_pFrame->ModifyHost((LONGLONG)it2->second->m_hClient);
					if (m_hTaskPane)
					{
						m_hTaskPane = NULL;
						m_pPresentation->m_pTaskPaneFrame->ModifyHost((LONGLONG)it2->second->m_hTaskPaneChildWnd);
		...
		void CPowerPntAddin::OnPresentationBeforeSave(/*[in]*/ _Presentation * Pres,	/*[in,out]*/ VARIANT_BOOL * Cancel) 
		{
			...
			if (m_pCurrentSavingPresentation)
			{
				...
				if (pFrame)
				{
					pParse = pFrame->UpdateWndNode();
					strXml = _GetDocXmlByKey(pCustomXMLParts.p, CComBSTR(L"Tangrams"));
					...
					if (m_Parse.LoadXml(strXml))
					{
						...
						if (pParse2)
						{
							...
							AddDocXml(Pres, CComBSTR(strXml), CComBSTR(L"Tangrams"));
						}
					}
				}
		...
		STDMETHODIMP CPowerPntAddin::CreateOfficeDocument(BSTR bstrXml)
		{
		...
		STDMETHODIMP CPowerPntAddin::SendUCMAMessage(BSTR bsipFrom, BSTR bsipTo, IDispatch* SenderDispObj, IWndNode* pSender, BSTR bstrMsg)
		{
			if (m_mapUCMABot.size())
			{
				...
				if (it != m_mapUCMABot.end())
				{
					...
					if (strTo.Find(_T("sip:")) != 0)
						strTo = _T("sip:") + strTo;
					LONGLONG h = 0;
					CComQIPtr<_Presentation> _pPresentation(SenderDispObj);
					if (_pPresentation)
					{
						CTangramPowerpntMessageObj* pObj = new CComObject<CTangramPowerpntMessageObj>;
		...
		STDMETHODIMP CPowerPntAddin::TangramCommand(IDispatch* RibbonControl)
		{
			...
			case 100:
			{
				CWndFrame* pFrame = pTangramPresentation->m_pFrame;
				if (pFrame == nullptr)
					break;
							...
							if (pCtrlDisp)
							{
								...
								if (pTangramPresentation->m_pTaskPanePage == nullptr)
								{
									...
									if (it != g_pTangram->m_mapWindowPage.end())
										pTangramPresentation->m_pTaskPanePage = it->second;
									else
									{
										pTangramPresentation->m_pTaskPanePage = new CComObject<CWndPage>();
										pTangramPresentation->m_pTaskPanePage->m_hWnd = hPWnd;
										g_pTangram->m_mapWindowPage[hPWnd] = pTangramPresentation->m_pTaskPanePage;
									}
			...
			if (it != m_mapOfficeObjects2.end())
			{
				if (m_pActivePowerPntObject != it->second)
				{
					...
					if (pTangramPresentation->m_pFrame)
						pTangramPresentation->m_pFrame->ModifyHost((LONGLONG)m_pActivePowerPntObject->m_hClient);

					if (pTangramPresentation->m_pTaskPaneFrame)
					{
						if (::IsWindow(m_pActivePowerPntObject->m_hTaskPane))
							pTangramPresentation->m_pTaskPaneFrame->ModifyHost((LONGLONG)m_pActivePowerPntObject->m_hTaskPane);
						else
							pTangramPresentation->m_pTaskPaneFrame->ModifyHost((LONGLONG)m_pActivePowerPntObject->m_hTaskPaneChildWnd);
					}
					CWndFrame* pFrame = m_pActivePowerPntObject->m_pPresentation->m_pFrame;
					if (pFrame)
					{
		...
		void CPowerPntAddin::ConnectOfficeObj(HWND hWnd)
		{
			...
			if (pDoc)
			{
				...
				if (strPath == _T(""))
				{
					...
					AddDocXml(pDoc, CComBSTR(pPowerPntWndObj->m_pPresentation->m_strDocXml), CComBSTR(L"Tangrams"));
							...
							if (pCtrlDisp)
							{
								...
								if (pTangramPresentation->m_pTaskPanePage == nullptr)
								{
									...
									else
									{
										pTangramPresentation->m_pTaskPanePage = new CComObject<CWndPage>();
										pTangramPresentation->m_pTaskPanePage->m_hWnd = hPWnd;
										m_mapWindowPage[hPWnd] = pTangramPresentation->m_pTaskPanePage;
									}
					...
					if (pParse)
					{
						...
						CreateWndPage((LONGLONG)pPowerPntWndObj->m_hForm, &pPage);
						pPowerPntWndObj->m_pPresentation->m_pDocPage = (CWndPage*)pPage;
						IWndFrame* pFrame = nullptr;
				...
				{
					CString strXML = GetDocXmlByKey(pDoc, CComBSTR(L"Tangrams"));
		...
		CTangramPowerpntMessageObj::CTangramPowerpntMessageObj()
		{
		...
		void CTangramPowerpntMessageObj::ProcessMsg(CString strMsg)
		{
