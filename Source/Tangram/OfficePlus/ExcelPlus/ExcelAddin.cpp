...
namespace OfficePlus
{
	namespace ExcelPlus
	{
		...
		STDMETHODIMP CExcelAddin::SendUCMAMessage(BSTR bsipFrom, BSTR bsipTo, IDispatch* SenderDispObj, IWndNode* pSender, BSTR bstrMsg)
		{
			if (m_mapUCMABot.size())
			{
				...
				if (it != m_mapUCMABot.end())
				{
					...
					if (strTo.Find(_T("sip:")) != 0)
						strTo = _T("sip:") + strTo;
					LONGLONG h = 0;
					CComQIPtr<_Workbook> pWorkbook(SenderDispObj);
					if (pWorkbook)
					{
						CTangramExcelMessageObj* pObj = new CComObject<CTangramExcelMessageObj>;
					...
					{
						return CTangram::SendUCMAMessage(bsipFrom, bsipTo, SenderDispObj, pSender, bstrMsg);
					}
				}
			}
			return S_OK;
		...
		STDMETHODIMP CExcelAddin::StartApplication(BSTR bstrAppID, BSTR bstrXml)
		{
			...
			if(strID.Find(_T("excel.application"))==0)
				return CreateOfficeDocument(bstrXml);
		...
		void CExcelAddin::OnWorkBookActivate(CExcelObject* pExcelObject)
		{
			...
			if (pWorkBook->m_pFrame)
				pWorkBook->m_pFrame->ModifyHost((LONGLONG)pExcelObject->m_hChildClient);
			if (pWorkBook->m_pTaskPaneFrame)
			{
				if (pExcelObject->m_hTaskPane)
				{
					pWorkBook->m_pTaskPaneFrame->ModifyHost((LONGLONG)pExcelObject->m_hTaskPane);
				}
				else 
					pWorkBook->m_pTaskPaneFrame->ModifyHost((LONGLONG)pExcelObject->m_hTaskPaneChildWnd);
			}
			...
			if (pFrame)
			{
				if (pFrame->m_bDesignerState&&m_pDesignerFrame&&m_pDesigningFrame != pFrame)
				{
					m_pDesigningFrame = pFrame;
		...
		void CExcelAddin::OnVbaFormInit(HWND hWnd, CWndFrame* pFrame)
		{
		...
		void CExcelAddin::UpdateOfficeObj(IDispatch* pObj, CString strXml, CString _strName)
		{
			...
			if (pForm)
			{
				...
				if (pWb)
				{
					...
					if (m_Parse.LoadXml(_GetDocXmlByKey(pCustomXMLParts.p, CComBSTR(L"Tangrams"))))
					{
						...
						if (pParse)
						{
							...
							AddDocXml(pWb, CComBSTR(strXml2), CComBSTR(L"Tangrams"));
						}
					}
				}
				return;
								...
								if (pParse)
								{
									...
									if (pParse)
									{
										...
										if (pParse2)
										{
											...
											AddDocXml(pWb, CComBSTR(str), CComBSTR(L"Tangrams"));
										}
									}
								}
							}
		...
		STDMETHODIMP CExcelAddin::TangramCommand(IDispatch* RibbonControl)
		{
			...
			//CString strDll = _T("tangramapp1");
			//LONGLONG llProxy = 0;
			//g_pTangram->LoadDocComponent(CComBSTR(strDll),&llProxy);
			//if (llProxy)
			//{
			...
			case 100:
			{
				...
				if (m_pWorkBook->m_pFrame)
				{
					...
					m_pDesigningFrame = m_pWorkBook->m_pFrame;
								...
								if (it == m_mapTaskPaneMap.end())
								{
									...
									if (pCtrlDisp)
									{
										...
										if (m_pWorkBook->m_pTaskPanePage == nullptr)
										{
											...
											m_Wnd.Detach();
											m_pWorkBook->m_pTaskPanePage = new CComObject<CWndPage>();
											m_pWorkBook->m_pTaskPanePage->m_hWnd = hPWnd;
											m_mapWindowPage[hPWnd] = m_pWorkBook->m_pTaskPanePage;
		...
		STDMETHODIMP CExcelAddin::CreateOfficeDocument(BSTR bstrXml)
		{
			...
			if (pWorkBooks)
			{
				...
				if(strXml!=_T(""))
					m_mapValInfo[_T("doctemplate")] = CComVariant(strXml);
				Unlock();
		...
		void CExcelAddin::ConnectOfficeObj(HWND hWnd)
		{
			...
			if (strPath == _T(""))
			{
				...
				AddDocXml(pWorkBook, CComBSTR(_pWorkBook->m_strDocXml), CComBSTR(L"Tangrams"));
		...
		void CExcelObject::OnOpenDoc()
		{
			...
			if (it != pAddin->m_mapWindowPage.end())
				m_pWorkBook->m_pDocPage = it->second;
			else
			{
				m_pWorkBook->m_pDocPage = new CComObject<CWndPage>();
				m_pWorkBook->m_pDocPage->m_hWnd = m_hClient;
				pAddin->m_mapWindowPage[m_hClient] = m_pWorkBook->m_pDocPage;
			}

			if (m_pWorkBook->m_pDocPage)
			{
				m_pWorkBook->m_pDocPage->put_External(m_pWorkBook->m_pWorkBook);
				IWndFrame* pFrame = nullptr;
				m_pWorkBook->m_pDocPage->CreateFrame(CComVariant(0), CComVariant((long)m_hChildClient), CComBSTR(L"Document"), &pFrame);
				...
				if (m_pWorkBook->m_pFrame)
				{
					...
					if (m_Parse.LoadXml(strXml))
					{
						...
						if (pParse)
						{
							...
							IWndNode* pNode = nullptr;
							m_pWorkBook->m_pFrame->Extend(strName.AllocSysString(), pParse2->xml().AllocSysString(), &pNode);
							CWndNode* _pNode = (CWndNode*)pNode;
							if (_pNode->m_pTangramNodeCommonData->m_pOfficeObj == nullptr&&pDisp)
							{
								_pNode->m_pTangramNodeCommonData->m_pOfficeObj = pDisp;
							...
							if (it == pAddin->m_mapTaskPaneMap.end())
							{
								...
								if (pCtrlDisp)
								{
									...
									if (m_pWorkBook->m_pTaskPanePage == nullptr)
									{
										...
										m_pWorkBook->m_pTaskPanePage = new CComObject<CWndPage>();
										m_pWorkBook->m_pTaskPanePage->m_hWnd = hPWnd;
										pAddin->m_mapWindowPage[hPWnd] = m_pWorkBook->m_pTaskPanePage;
		...
		void CExcelObject::ProcessMouseDownMsg(HWND hWnd)
		{
			...
			if (hActiveWnd == m_hForm)
			{
				...
				{
					if (g_pTangram->m_pActiveNode->m_nViewType == ViewType::Splitter || g_pTangram->m_pActiveNode->m_nViewType == ViewType::TangramTreeView)
						return;
		...
		void CExcelObject::OnObjDestory()
		{
			if (m_pOfficeObj != nullptr)
			{
				...
				if (nCount > 0 && m_pWorkBook->m_pFrame)
				{
					...
					m_pWorkBook->m_pFrame->ModifyHost((LONGLONG)::GetParent(it2->second->m_hChildClient));
					if (m_hTaskPane)
					{
						m_hTaskPane = NULL;
		...
		CTangramExcelMessageObj::CTangramExcelMessageObj()
		{
		...
		void CTangramExcelMessageObj::ProcessMsg(CString strMsg)
		{
