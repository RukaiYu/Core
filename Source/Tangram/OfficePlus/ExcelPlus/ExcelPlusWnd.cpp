	...
	namespace ExcelPlus
	{
		...
		void CExcelWorkBook::InitWorkBook()
		{
			...
			auto t = create_task([pAddin, this]()
			{
				...
				if (m_strDocXml==_T(""))
				{
					m_strDocXml = pAddin->GetDocXmlByKey(m_pWorkBook, CComBSTR(L"Tangrams"));
				...
				{
					pAddin->AddDocXml(m_pWorkBook, CComBSTR(m_strDocXml), CComBSTR(L"Tangrams"));
				}
				CoUninitialize();
			}).then([pAddin,this]()
			{
		...
		void CExcelWorkBook::OnNewSheet(IDispatch* Sh)
		{
			...
			if (m_bCreating==false)
			{
				...
				if (m_Parse.LoadXml(strTemplate) || m_Parse.LoadFile(strTemplate))
				{
					...
					strSheetXml += strName;
					strSheetXml += _T(">");
					if (::MessageBox(::GetActiveWindow(), _T("Do you want to create a taskpane Object with this sheet?"), _T("New Sheet"), MB_YESNO) == IDYES)
					{
		...
		void CExcelWorkBook::OnBeforeSave(VARIANT_BOOL SaveAsUI, VARIANT_BOOL* Cancel)
		{
			...
			if (m_pFrame)
			{
				...
				{
					CString strXml = pAddin->GetDocXmlByKey(m_pWorkBook, CComBSTR(L"Tangrams"));
					CTangramXmlParse m_Parse;
					...
					if (it == pAddin->m_mapTaskPaneMap.end())
					{
						...
						if (pCtrlDisp)
						{
							...
							if (m_pTaskPanePage == nullptr)
							{
								...
								m_pTaskPanePage = new CComObject<CWndPage>();
								m_pTaskPanePage->m_hWnd = hPWnd;
								pAddin->m_mapWindowPage[hPWnd] = m_pTaskPanePage;
