	...
	if (lParam == 19922017)
	{
		...
		if (pPage)
		{
			...
			if ((::GetWindowLong(m_hWnd, GWL_EXSTYLE)&WS_EX_MDICHILD))
			{
				...
				if (g_pTangram->m_pMDIMainWnd->m_pPage&&m_pDocTemplate->m_strKey != _T(""))
				{
...
CTangramDocWnd::~CTangramDocWnd(void)
{
	::PostMessage(g_pTangram->m_hTangramWnd, WM_TANGRAMMSG, 0, 10001000);
}
...
int CWndFrame::Dispatch(CString strChannel, CString strArg1, CString strArg2)
{
	int index = 0;
