...
//#endif
		...
		if (::IsWindow(m_hVSToolBoxWnd))
		{
			if (m_pTangramPackageProxy->m_pToolBoxFrame)
			{
				...
				if (it != m_mapValInfo.end())
				{
					...
					m_pTangramPackageProxy->m_pToolBoxFrame->Extend(CComBSTR("tangramtoolbox"), it->second.bstrVal, &pNode);
				}
	...
	STDMETHODIMP CVSAddin::put_AppKeyValue(BSTR bstrKey, VARIANT newVal)
	{
		...
		if (strKey.CompareNoCase(_T("TangramPackageProxy")) == 0)
		{
			if (newVal.llVal)
			{
				...
				m_pTangramPackageProxy->m_pProxy = (CTangramProxyBase*)this;
	...
	void CVSAddin::TangramInit()
	{
		...
		if (m_pTangramPackageProxy&&::IsWindow(m_pTangramPackageProxy->m_hTangramToolWnd))
		{
			...
			if (it != m_mapWindowPage.end())
			{
				...
				m_pPage->CreateFrame(CComVariant((LONGLONG)::GetParent(m_pTangramPackageProxy->m_hTangramToolWnd)), CComVariant((LONGLONG)m_pTangramPackageProxy->m_hTangramToolWnd), CComBSTR("TangramToolBar"), &pFrame);
				...
				if (::PathFileExists(strDataPath))
				{
					...
					pFrame->ExtendFromWeb(CComBSTR(strURL), CComBSTR(strKey), CComBSTR(strOrg), CComBSTR(strRepo), CComBSTR(strBranch), CComBSTR(strPath));
				}
				else
				{
					...
					{
						IWndNode* pNode = nullptr;
						pFrame->Extend(CComBSTR("default"), it->second.bstrVal, &pNode);
					}
				}
			}
		}
	...
	STDMETHODIMP CVSExtender::ExtendXmlUI(BSTR bstrKey, BSTR bstrXml, IWndNode** ppNode)
	{
		...
		CTangramXmlParse m_Parse;
		if(pVSAddin->m_pTangramPackageProxy->m_pFrame&&(m_Parse.LoadFile(strXml)|| m_Parse.LoadXml(strXml)))
		{
			if (m_Parse.GetChild(_T("window")))
			{
				pVSAddin->m_pTangramPackageProxy->m_pFrame->Extend(bstrKey, bstrXml, ppNode);
			}
		}
