...
LRESULT CSplitterNodeWnd::OnSplitterNodeAdd(WPARAM wParam, LPARAM lParam)
{
	...
	if (lParam == 1993)
	{
		...
		case 3:
		{
			//CWndNode* pNode = m_pWndNode->m_pTangramNodeCommonData->m_pFrame->m_pWorkNode;
...
LRESULT CSplitterNodeWnd::OnActiveTangramObj(WPARAM wParam, LPARAM lParam)
{
	...
	//::InvalidateRect(m_pWndNode->m_pFrame->m_hWnd, nullptr, true);
	return -1;
}
...
void CSplitterNodeWnd::StopTracking(BOOL bAccept)
{
	...
	if (bAccept)
	{
		...
		pFrame->UpdateVisualWPFMap(::GetParent(m_hWnd), false);
		//for (auto it : pFrame->m_mapVisibleWPFView)
		//{
		//	it.second->m_pTangramWPFObj->InvalidateVisual();
		//}
		::InvalidateRect(pFrame->m_hWnd, nullptr, true);
		if (pFrame->m_bDesignerState&&g_pTangram->m_pDesignWindowNode)
		{
...
void CSplitterNodeWnd::RecalcLayout()
{
	...
	if (m_bCreated == false || GetDlgItem(IdFromRowCol(0, 0)) == NULL)
		return;
	_RecalcLayout();
	m_pWndNode->m_pTangramNodeCommonData->m_pFrame->UpdateVisualWPFMap(m_hWnd, false);
}
...
BOOL CSplitterNodeWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	...
	if (CreateStatic(pParentWnd, m_pWndNode->m_nRows, m_pWndNode->m_nCols, dwStyle, nID))
	{
		...
		if (m_pWndNode->m_pTangramNodeCommonData->m_pPage)
			m_pWndNode->m_pTangramNodeCommonData->m_pPage->Fire_NodeCreated(m_pWndNode);
		...
		if (pFrame->m_pBindingNode)
		{
			pHostNode = pFrame->m_pBindingNode;
...
LRESULT CSplitterNodeWnd::OnWndNodeIPCMsg(WPARAM wParam, LPARAM lParam)
{
	if (lParam)
	{
